        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                /*for (var i = (pageNo - 1) * pageSize; i < pageNo * pageSize; i++) {
                    var t = data.article_list[i];
                    if (t != null && t.article_type == '문의') {
                        view += '<tr>' +
                            '    <td>' + t.seq + '</td>' +
                            '       <td><a id="question' + t.seq + '" class="read" href="javascript:void(0)">' + t.title + '</a></td>' +
                            '       <td>' + t.customer_id + '</td>' +
                            '       <td>' + t.reg_date + '</td>' +
                            '       <td>' + t.answer + '</td>' +
                            '       <td><input id="delete' + t.seq + '" class="delete" type="button" value="x"/></td>' +
                ...
                var pagination = '<table>' +
                    ...
                    '                  </tr>' +
                    '               </table>';
                $('#faq_pagination').html(pagination);
                pageCount = isMultiply5(articleCount) ? articleCount / blockSize : articleCount / blockSize + 1;
                if (pageCount > pageSize) {
                    $('#goLastPage').before('<a id="goNextPage" href="javascript:void(0)"><img src="' + $.context() + '/resources/img/pagination/next.gif" alt="" /></a>');
                }
                if (pageNo > blockSize) {
                    $('#goFirstPage').after('<a id="goPrevPage" href="javascript:void(0)"><img src="' + $.context() + '/resources/img/pagination/prev.gif" alt="" /></a>');
                }

                var pageNumbers = '';
                for (var i = 1; i <= pageCount; i++) {
                    pageNumbers += '<a id="page' + i + '" class="goPage" href="javascript:void(0)">' + i + '</a>';
                }
                $('#pages').append(pageNumbers);
                $('#page' + pageNo).css('background', '#2b2929').css('color', 'white');
                $('.goPage').on('click', function() {
                    var page = $(this).attr('id').split('page')[1];
                    adminBbsFaq(page);
                });
                ...
                $('#goPrevPage').on('click', function() {
                    var prev = pageNo - pageSize;
                    adminBbsFaq(prev);
                });
                $('#goNextPage').on('click', function() {
                    var next = ((pageNo + pageSize) <= Math.floor(articleCount / blockSize) + 1) ? pageNo + pageSize : Math.floor(articleCount / blockSize) + 1;
                    if (isMultiply5(articleCount)) {
                        next -= 1;
                    }
                    adminBbsFaq(next);
                });
                $('#goLastPage').on('click', function() {
                    var last = Math.floor(articleCount / blockSize) + 1;
                    if (isMultiply5(articleCount)) {
                        last -= 1;
                    }
                    adminBbsFaq(last);
